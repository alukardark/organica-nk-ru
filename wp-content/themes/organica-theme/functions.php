...
add_action( 'template_redirect', function() {
    if ( preg_match( '#^/show.php\?txt=5/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/contact/', 301 );
        exit;
    }
    if ( preg_match( '#^/news.php/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/company/news/', 301 );
        exit;
    }
    if ( preg_match( '#^/show.php\?txt=2/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/products/', 301 );
        exit;
    }
    if ( preg_match( '#^/show.php\?txt=44/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/products/', 301 );
        exit;
    }
    if ( preg_match( '#^/show.php\?txt=47/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/products/', 301 );
        exit;
    }
    if ( preg_match( '#^/faq.php/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/contact/', 301 );
        exit;
    }
    if ( preg_match( '#^/show.php\?new&id=27/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/products/dimebon/', 301 );
        exit;
    }
    if ( preg_match( '#^/show.php\?txt=3/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/spisok-iii/', 301 );
        exit;
    }
     if ( preg_match( '#^/show.php\?txt=38/?$#i', $_SERVER['REQUEST_URI'] ) ) {
        wp_redirect( 'http://organica-nk.ru/organika-segodnya/', 301 );
        exit;
    }
